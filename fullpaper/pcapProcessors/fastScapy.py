...
def rdpcap(filename, count=-1):
    ...
    with PcapReader2(filename) as fdesc:  # type: ignore
...
class PcapReader2_metaclass(type):
    ...
    def __new__(cls, name, bases, dct):
        ...
        """
        newcls = super(PcapReader2_metaclass, cls).__new__(
            cls, name, bases, dct
        )
...
class RawPcapReader2(metaclass=PcapReader2_metaclass):
    """A stateful pcap reader. Each packet is returned as a string"""

    # TODO: use Generics to properly type the various readers.
    # As of right now, RawPcapReader2 is typed as if it returned packets
    ...
    def __next__(self):
        # type: () -> Tuple[bytes, RawPcapReader2.PacketMetadata]
        """
    ...
    def _read_packet(self, size=MTU):
        # type: (int) -> Tuple[bytes, RawPcapReader2.PacketMetadata]
        ...
        return (self.f.read(caplen)[:size],
                RawPcapReader2.PacketMetadata(sec=sec, usec=usec,
                                             wirelen=wirelen, caplen=caplen))

    def read_packet(self, size=MTU):
        # type: (int) -> Packet
        raise Exception(
            "Cannot call read_packet() in RawPcapReader2. Use "
            "_read_packet()"
        )

    def dispatch(self,
                 callback  # type: Callable[[Tuple[bytes, RawPcapReader2.PacketMetadata]], Any]  # noqa: E501
                 ):
        # type: (...) -> None
...
class PcapReader2(RawPcapReader2):
    def __init__(self, filename, fdesc=None, magic=None):  # type: ignore
        # type: (str, IO[bytes], bytes) -> None
        RawPcapReader2.__init__(self, filename, fdesc, magic)
        try:
    ...
    def read_packet(self, size=MTU, **kwargs):
        # type: (int, **Any) -> Packet
        rp = super(PcapReader2, self)._read_packet(size=size)
        if rp is None:
            raise EOFError
        s, pkt_info = rp
    ...
    def goForward(self, num_packets):
        # type: (int) -> None
        """
        Move the file pointer forward by the specified number of packets.
        """
        for _ in range(num_packets):
            try:
                self._read_packet()
            except EOFError:
                raise Scapy_Exception("Cannot go forward. End of file reached.")
...
class RawPcapNgReader(RawPcapReader2):
    """A stateful pcapng reader. Each packet is returned as
    bytes.

    """

    alternative = RawPcapReader2  # type: Type[Any]
...
class PcapNgReader(RawPcapNgReader, PcapReader2):

    alternative = PcapReader2
...
class ERFEthernetReader_metaclass(PcapReader2_metaclass):
    def __call__(cls, filename):
        # type: (Union[IO[bytes], str]) -> Any
...
class ERFEthernetReader(PcapReader2,
...
def tcpdump(
    ...
    if flt is not None:
        ...
        if linktype is None and isinstance(pktlist, str):
            ...
            with PcapReader2(pktlist) as rd:
